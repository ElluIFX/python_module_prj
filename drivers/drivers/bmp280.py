...
class BMP280:
    def __init__(self, i2c_addr=I2C_ADDRESS_VCC, i2c_dev=None):
        self._calibration = BMP280Calibration()
        ...
        if self._i2c_dev is None:
            try:
                ...
                self._i2c_dev = SMBus(3)
            except ImportError:
                raise RuntimeError("Please install smbus2")
        self._bmp280 = Device(
            ...
            registers=(
                ...
                Register(
                    ...
                    fields=(
                        BitField(
                            "dig_t1", 0xFFFF << 16 * 11, adapter=U16Adapter()
                        ),  # 0x88 0x89
                        BitField(
                            "dig_t2", 0xFFFF << 16 * 10, adapter=S16Adapter()
                        ),  # 0x8A 0x8B
                        BitField(
                            "dig_t3", 0xFFFF << 16 * 9, adapter=S16Adapter()
                        ),  # 0x8C 0x8D
                        BitField(
                            "dig_p1", 0xFFFF << 16 * 8, adapter=U16Adapter()
                        ),  # 0x8E 0x8F
                        BitField(
                            "dig_p2", 0xFFFF << 16 * 7, adapter=S16Adapter()
                        ),  # 0x90 0x91
                        BitField(
                            "dig_p3", 0xFFFF << 16 * 6, adapter=S16Adapter()
                        ),  # 0x92 0x93
                        BitField(
                            "dig_p4", 0xFFFF << 16 * 5, adapter=S16Adapter()
                        ),  # 0x94 0x95
                        BitField(
                            "dig_p5", 0xFFFF << 16 * 4, adapter=S16Adapter()
                        ),  # 0x96 0x97
                        BitField(
                            "dig_p6", 0xFFFF << 16 * 3, adapter=S16Adapter()
                        ),  # 0x98 0x99
                        BitField(
                            "dig_p7", 0xFFFF << 16 * 2, adapter=S16Adapter()
                        ),  # 0x9A 0x9B
                        BitField(
                            "dig_p8", 0xFFFF << 16 * 1, adapter=S16Adapter()
                        ),  # 0x9C 0x9D
                        BitField(
                            "dig_p9", 0xFFFF << 16 * 0, adapter=S16Adapter()
                        ),  # 0x9E 0x9F
                    ),
                    bit_width=192,
                ),
    ...
    def setup(
        ...
        try:
            ...
            if chip.id != CHIP_ID:
                raise RuntimeError(
                    "Unable to find bmp280 on 0x{:02x}, CHIP_ID returned {:02x}".format(
        ...
        self._calibration.set_from_namedtuple(self._bmp280.get("CALIBRATION"))
    ...
    def measure(self):
        ...
        self._temperature = self._calibration.compensate_temperature(raw.temperature)
        self._pressure = self._calibration.compensate_pressure(raw.pressure) / 100.0
    ...
    def get_altitude(self, qnh=1013.25, manual_temperature=None) -> float:
        ...
        altitude = (
            (pow((qnh / pressure), (1.0 / 5.257)) - 1) * (temperature + 273.15)
        ) / 0.0065
        return altitude
