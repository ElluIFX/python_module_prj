...
class VL53L0X:
    ...
    def __init__(self, address: int = 41, io_timeout_s: float = 1) -> None:
        self._bus = request_interface("i2c", "VL53L0X", address)
        ...
        ref_spad_map[1:6] = self._bus.read_reg_data(ref_spad_map[0], 6)
        ...
        for i in range(48):
            if i < first_spad_to_enable or spads_enabled == spad_count:
                ...
                spads_enabled += 1
        self._bus.write_reg_data(ref_spad_map[0], ref_spad_map[1:6])
        for pair in (
    ...
    def _read_u8(self, address: int) -> int:
        ...
        return self._bus.read_reg_byte(address)

    def _read_u16(self, address: int) -> int:
        # Read a 16-bit BE unsigned value from the specified 8-bit address.
        buf = self._bus.read_reg_data(address, 2)
    ...
    def _write_u16(self, address: int, val: int) -> None:
        ...
        buf[0] = (val >> 8) & 0xFF
        buf[1] = val & 0xFF
        self._bus.write_reg_data(address, buf)
    ...
    def _get_spad_info(self) -> Tuple[int, bool]:
        ...
        while self._read_u8(0x83) == 0x00:
            if (
                self._io_timeout_s > 0
                and (time.monotonic() - start) >= self._io_timeout_s
            ):
    ...
    def _get_sequence_step_timeouts(
        ...
        pre_range_us = _timeout_mclks_to_microseconds(
            round(pre_range_mclks), pre_range_vcsel_period_pclks
        )
        ...
        final_range_us = _timeout_mclks_to_microseconds(
            round(final_range_mclks), final_range_vcsel_period_pclks
        )
        return (
            msrc_dss_tcc_us,
            pre_range_us,
    ...
    def distance(self) -> float:
        ...
        the sensor and return the distance in meters.
        """
        return self.range / 1000.0
    ...
    def continuous_mode(self, mode: bool) -> None:
        """Set the sensor into or out of continuous mode."""
        if mode:
    ...
    def set_address(self, new_address: int) -> None:
        ...
        self._bus.address = new_address

    @property
    def address(self) -> int:
        """The I2C address of the VL53L0X sensor. Defaults to ``0x29``."""
